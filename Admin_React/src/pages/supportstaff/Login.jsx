...
const SupportStaffLogin = () => {
    ...
    const { loginSupportStaff, user } = useAuth();
    const hasNavigatedRef = useRef(false);
    const isInitialMountRef = useRef(true);

    // Reset refs when component mounts (in case of remount)
    useEffect(() => {
        // Reset navigation ref on mount to allow fresh check
    ...
    useEffect(() => {
        ...
        // Don't redirect if tokens were just cleared (coming from profile edit)
        if (ssToken && ssRole === 'SUPPORTSTAFF' && user && user.role === 'SUPPORTSTAFF') {
    ...
    useEffect(() => {
        ...
        // Only navigate if:
        // 1. User state exists and is SUPPORTSTAFF
        // 2. We have a token in localStorage (user just logged in)
        // 3. We haven't navigated yet
        // 4. We're still on login page
        const ssToken = localStorage.getItem('ssToken');
        if (user && user.role === 'SUPPORTSTAFF' && ssToken && !hasNavigatedRef.current && location.pathname === '/support/login') {
            hasNavigatedRef.current = true;
    ...
    const validate = () => {
        const newErrors = {};
        if (!employeeId.trim()) newErrors.employeeId = 'Please fill out this field.';
    ...
    const handleLogin = async (e) => {
        ...
        try {
            const response = await apiClient.post(`/supportstaff/loginSupportStaff?username=${encodeURIComponent(employeeId)}&password=${encodeURIComponent(password)}`);
            const data = response.data;

            if (data.status && data.data) {
                loginSupportStaff(data.data.token, data.data.username, data.data.role);
                if (rememberMe) localStorage.setItem('ssRememberMe', 'true');
                ...
                setErrorMessage(data.message || 'Access denied. Check your ID or Password.');
            }
        } catch (error) {
    ...
    return (
        <div className="h-screen flex items-center justify-center bg-[#0f172a] px-5 py-12 font-['Plus_Jakarta_Sans',sans-serif] overflow-hidden">
            <div className="w-full max-w-[1050px] bg-white rounded-[40px] overflow-y-auto p-5 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] flex max-h-[calc(100vh-6rem)]">
                <div className="flex w-full gap-8">
                    ...
                    <section className="flex-1 bg-black bg-cover bg-center rounded-[32px] p-12 flex flex-col justify-center text-white relative overflow-hidden hidden lg:flex"
                        style={{ backgroundImage: "linear-gradient(to bottom, rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.8)), url('https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&q=80&w=1000')" }}>
                        <div className="absolute bottom-[-10%] left-[-10%] w-[120%] h-[60%] bg-[radial-gradient(circle_at_center,rgba(37,99,235,0.4)_0%,transparent_70%)] blur-[40px] z-10" />
                        ...
                        <div className="w-full max-w-[380px]">
                            <header className="mb-7 flex flex-col items-center">
                                ...
                                <div className="w-full text-center">
                                    <h2 className="text-[17px] font-semibold text-[#374151] leading-[1.2]">Welcome to SaHarsh — Support Staff Portal</h2>
                                </div>
                            </header>
                            ...
                            <form onSubmit={handleLogin} className="flex flex-col gap-5" noValidate>
                                <div className="flex flex-col gap-2 relative">
                                    <label className="text-[14px] font-semibold text-[#1e293b]">Employee ID</label>
                                    <div className="relative">
                                        <input
                                            ...
                                            className={`w-full px-4 py-3.5 bg-[#f8fafc] border-[1.5px] rounded-xl text-[15px] transition-all duration-200 text-[#1e293b] focus:border-[#2563eb] focus:bg-white focus:outline-none focus:ring-4 focus:ring-[#7c3aed]/10 ${errors.employeeId ? 'border-[#f97316] bg-[#fffaf0]' : 'border-[#e2e8f0]'}`}
                                        />
                                        {errors.employeeId && (
                                ...
                                <div className="flex flex-col gap-2 relative">
                                    <label className="text-[14px] font-semibold text-[#1e293b]">Password</label>
                                    <div className="relative">
                                        <input
                                            ...
                                            placeholder="••••••••••••"
                                            className={`w-full px-4 py-3.5 bg-[#f8fafc] border-[1.5px] rounded-xl text-[15px] transition-all duration-200 text-[#1e293b] focus:border-[#2563eb] focus:bg-white focus:outline-none focus:ring-4 focus:ring-[#7c3aed]/10 ${errors.password ? 'border-[#f97316] bg-[#fffaf0]' : 'border-[#e2e8f0]'}`}
                                        />
                                        <button
                                            type="button"
                                            className="absolute right-3 top-1/2 -translate-y-1/2 bg-transparent border-none text-[#64748b] cursor-pointer hover:text-[#2563eb] p-2 flex items-center justify-center"
                                ...
                                <div className="flex items-center justify-between mt-1">
                                    <label className="flex items-center gap-2 cursor-pointer text-sm font-medium text-[#64748b]">
                                        <input
                                            type="checkbox"
                                            checked={rememberMe}
                                ...
                                <button type="submit" className="w-full p-3 bg-[#2563eb] text-white border-none rounded-xl text-sm font-semibold cursor-pointer transition-all duration-200 mt-[10px] flex items-center justify-center gap-3 hover:bg-[#1d4ed8] hover:-translate-y-px active:translate-y-0 shadow-lg shadow-blue-500/10 disabled:opacity-70 disabled:cursor-not-allowed" disabled={isLoading}>
                                    {isLoading ? (
                                        <>
                            ...
                            <p className="mt-8 text-center text-sm text-[#64748b]">
                                Technical issues? <a href="#" className="text-[#000000] font-bold hover:underline">Contact IT Admin</a>
                            </p>
