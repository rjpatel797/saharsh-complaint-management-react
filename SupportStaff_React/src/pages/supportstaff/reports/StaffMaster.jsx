...
const StaffMaster = () => {
    ...
    // Filter State - Input dates (for UI)
    ...
    useEffect(() => {
        ...
        // Use applied dates (only set when Apply Filter is clicked)
        fetchReport(currentPage, pageSize, appliedFromDate, appliedToDate);
    }, [currentPage, pageSize, appliedFromDate, appliedToDate]); // Removed fetchReport from deps - it's stable
    ...
    const exportToExcel = async () => {
        ...
        try {
            ...
            let url = `/report/staffMasterReport?page=0&size=10000`;
            if (appliedFromDate && appliedToDate) {
                url += `&fromDate=${encodeURIComponent(appliedFromDate)}&toDate=${encodeURIComponent(appliedToDate)}`;
            }
    ...
    const handleApplyFilter = () => {
        ...
        if (fromDate || toDate) {
            ...
            // Set applied dates - this will trigger API call via useEffect
            setAppliedFromDate(fromDate);
            setAppliedToDate(toDate);
        } else {
            // If no dates provided, clear applied dates (fetch all data)
            setAppliedFromDate('');
            setAppliedToDate('');
        }
        setCurrentPage(0); // Reset to first page when applying filters
        // The useEffect will trigger fetchReport when applied dates change
    };
    ...
    const clearFilters = () => {
        ...
        setSearchTerm('');
        setCurrentPage(0);
        // The useEffect will trigger fetchReport when applied dates are cleared
    };
    ...
    return (
        <div className="space-y-3 animate-fade-in text-gray-800 pt-2">
            {/* Header & Main Actions */}
            ...
            {/* Report Table — same card shell as Ticket Master (rounded border + scroll + Pagination) */}
                                ...
                                currentRecords.map((r, idx) => {
                                    ...
                                    return (
                                        <tr key={idx} className="group hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-indigo-50/50 transition-all duration-200 border-b border-gray-100">
                                            <td className="text-gray-700 whitespace-nowrap text-center py-2 px-3 font-medium text-sm">{srNo}</td>
